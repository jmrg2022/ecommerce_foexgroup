...
class UserProfile(models.Model):
    ...
    cpf = models.CharField("DNI", max_length=10)
...
class Item(models.Model):
    ...
    def __str__(self):
        return "Código:" + self.title + "  <<<--->>>  " + self.description + "  <<<--->>> Precio:" + str(self.price) + "  <<<--->>> Precio Descuento:" + str(self.discount_price) + "  <<<--->>> Cantidad en Stock:" + str(self.quantity)
    def get_url(self):
...
class Order(models.Model):
    ...
    payment = models.ForeignKey('mercadopago_payment.Payment', verbose_name="FormaPago", related_name="mercadopago",
                                on_delete=models.CASCADE,
                                blank=True, null=True)
    coupon = models.ForeignKey('Coupon', verbose_name="Cupón",
                                on_delete=models.CASCADE,
                                blank=True, null=True)
    on_the_road = models.BooleanField("En Distribución", default=False)
    delivered = models.BooleanField("Entregado", default=False)
    refund_requested = models.BooleanField("Renuncia Solicitada", default=False)
    refund_accepted = models.BooleanField("Renuncia Aceptada", default=False)
